<?php
	...
	$polygon = isset($request->polygon)? JSON_decode($request->polygon) : "";
...
/*$a ='{
  "geo_polygon": {
	"coordinates": {
	  "points": [
		{"lat": -33.2962,"lon": -70.76089},
		{"lat": -33.30826,"lon": -70.41275},
		{"lat": -33.57318,"lon": -70.46145},
		{"lat": -33.55063,"lon": -70.77351},
		{"lat": -33.2962,"lon": -70.76089}
	  ]
	},
...
if(!empty($device)) {
	...
	for($i=0; $i<count($tmparr); $i++) {
		$more = $more.', {"match": { "device_type.keyword": "'.$tmparr[$i].'" }}';
	}

	array_push($filter, json_decode( '{"bool": {"should": [{"match": { "device_type.keyword": "'.$tmparr[0].'" }}'.$more.'],"minimum_should_match": 1}}', true));
}
if(!empty($sentiment)) {
	$tmparr = $sentiment;
	$more="";
